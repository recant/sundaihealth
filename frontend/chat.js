...
function dashboardSnapshot(){
  ...
  return {
    ...
    population:{headline:text('simpleTitle')||null,summary:text('simpleBody')||null,nearest_people:collectList('#simpleCaseList .simple-case-button',4),selected_case:collectList('#simpleCaseDetail .simple-time-row',5),source:'Mishra et al., Nature Biomedical Engineering (2020), doi:10.1038/s41551-020-00640-6'},
    ...
    important_interpretation:'PulseLab predicts expected blood-testing yield and a persistence scenario. The population page uses real published illness timelines, but its match percentage is demo pattern-matching rather than a validated disease-risk score. PulseLab is not a diagnosis, a validated doctor/ER triage score, or currently a validated future-disease-incidence model.'
  };
}
...
function localFallback(message,snapshot=dashboardSnapshot()){
  ...
  if(/(similar|analog|people like|cohort|population)/i.test(q)&&snapshot.population?.summary){const people=snapshot.population.nearest_people?.length?` Closest real study cases shown: ${snapshot.population.nearest_people.join(' ')}`:'';return `${snapshot.population.headline}. ${snapshot.population.summary}${people} The illness stories come from ${snapshot.population.source}; the displayed match percentage is demo math, not a medical risk score.`;}
