...
function dashboardSnapshot(){
  ...
  return {
    ...
    population:{headline:text('populationHeadline')||null,summary:text('populationSummary')||null,nearest_people:collectList('#analogPeople .analog-person',5),outcomes:collectList('#outcomeBars .outcome-row',4)},
...
function localFallback(message,snapshot=dashboardSnapshot()){
  ...
  if(/(similar|analog|people like|cohort|population)/i.test(q)&&snapshot.population?.summary)return `${snapshot.population.headline}. ${snapshot.population.summary}`;
