...
def build_examples() -> pd.DataFrame:
    ...
    gly = labs['LBXGH'].ge(5.7)
    cbc_bad = (
        labs['LBXWBCSI'].lt(4.0) | labs['LBXWBCSI'].gt(11.0) |
        labs['LBXPLTSI'].lt(150) | labs['LBXPLTSI'].gt(450) |
        (male & (labs['LBXHGB'].lt(13.0) | labs['LBXHGB'].gt(17.5))) |
        (~male & (labs['LBXHGB'].lt(12.0) | labs['LBXHGB'].gt(15.5)))
    )
    metabolic = (
        labs['LBXSAL'].lt(3.5) |
        labs['LBXSCR'].gt(np.where(male,1.3,1.1)) |
        labs['LBXSATSI'].gt(np.where(male,55,45)) |
        labs['LBXSASSI'].gt(40) |
        labs['LBXSKSI'].lt(3.5) | labs['LBXSKSI'].gt(5.1) |
        labs['LBXSNASI'].lt(135) | labs['LBXSNASI'].gt(145)
    )
    lipid = labs['LBDHDD'].lt(np.where(male,40,50)) | labs['LBXSCH'].ge(240)
    ...
    for seqn,g in pax.groupby('SEQN'):
        ...
        g=g[g['PAXVMD'].fillna(0)>=1000]
        if len(g)<3: continue
        sleep=g['PAXSWMD']/60.0
        act=g['PAXMTSD'].astype(float)
        for i in range(1,len(g)):
            prev_sleep=sleep.iloc[:i]
            prev_act=act.iloc[:i]
            if prev_sleep.notna().sum()<1: continue
            base_sleep=float(prev_sleep.median())
            cur_sleep=float(sleep.iloc[i])
            base_act=float(prev_act.median()) if prev_act.notna().any() else np.nan
            cur_act=float(act.iloc[i])
            sign=np.sign(cur_sleep-base_sleep)
            persistence=1
            for j in range(i-1,0,-1):
                b=float(sleep.iloc[:j].median())
                s=np.sign(float(sleep.iloc[j])-b)
    ...
    x=x.rename(columns={'RIDAGEYR':'age'})
    x['sex_male']=x['RIAGENDR'].eq(1).astype(float)
...
def train():
    ...
    probs=pipe.predict_proba(X.iloc[te]); metrics={}
